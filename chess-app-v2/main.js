...
function getKingLastMoves(color) {
...
function drop(ev) {
    ...
    if ((squareContent.pieceColor == "blank") && (legalSquares.includes(destinationSquareId)) && data) {
...
function isMoveValidAgainstCheck(legalSquares, startingSquareId, pieceColor, pieceType) {
    ...
    let kingSquare = isWhiteTurn ? getKingLastMoves("white") : getKingLastMoves("black");
...
function createChessPiece(pieceType, color, pieceClass) {
    ...
    let pieceName = "pieces/" + color.charAt(0).toUpperCase() + color.slice(1) + "-" + pieceType.charAt(0).toUpperCase() + pieceType.slice(1) + ".png";
...
function generateFEN(boardSquares) {
    ...
    let shortCastlePossibleForBlack = !kingHasMoved("black") && rookHasMoved("black", "h1")
    let longCastlePossibleForBlack = !kingHasMoved("black") && rookHasMoved("black", "a1")
    ...
    let fiftyMovesRuleCounter = getFiftyMovesRuleCount();
    fen += " " + fiftyMovesRuleCounter;
